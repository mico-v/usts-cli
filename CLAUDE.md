...
- **`commands/`** — CLI orchestration/presentation. `ensureSession()` validates proactively (and auto-relogins) instead of only restoring locally. `commands/help.ts` is the **single home for user-facing documentation**: each command's `--help` tail (`COMMAND_HELP`) and the long-form topics behind `usts help <topic>` (`HELP_TOPICS`). The README is deliberately a short landing page — new usage detail goes in `help.ts`, not the README, and nowhere in both.
...
6. Register it in `index.ts` (with its `COMMAND_HELP` entry) and add fixture/CLI tests.
