 ...
 */
import { LoginResponse, ScoreItem, ScheduleItem, ProfileInfo, ExamItem, CourseListItem, SelectOption, ClassScheduleQuery, ClassScheduleItem, ClassScheduleView, NotificationItem, GpaSummary, AcademiaSummary, AcademiaCourseItem, SelectedCourseItem } from '../types/api';
...
import { AuthenticatedTransport } from '../infrastructure/http/authenticated-transport';
import { OperationEffect, RetryNotice } from '../infrastructure/http/transport';
import { loginViaScript as loginRequest, probeSession as probeRequest } from '../infrastructure/jwgl/auth';
import {
...
} from '../infrastructure/jwgl/records';
import {
...
} from '../infrastructure/jwgl/schedule';
import {
  queryAcademia as fetchAcademia, queryAcademiaCategory as fetchAcademiaCategory, queryGpa as fetchGpa,
} from '../infrastructure/jwgl/academia';
import { downloadAcademiaPdf as fetchAcademiaPdf, downloadSchedulePdf as fetchSchedulePdf } from '../infrastructure/jwgl/document-api';
...
export interface JwglClientOptions extends BaseUrlPolicy {
  sessionStore?: SessionStore;
  sessionManager?: SessionManager;
  /**
   * 网络重试等进度提示的输出通道。默认写 stderr（诊断信息不进 stdout 的结果/JSON）。
   * 做成可注入是为了让测试静音，也方便库用法自行接管。
   */
  onNotice?: (message: string) => void;
}

export class JwglClient implements SessionPort {
